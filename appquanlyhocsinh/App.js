...
class HomeScreen extends React.Component {
  ...
  handlePhone = (text) => {
   ...
   login = (username, email, phone) => {
    fetch('http://192.168.1.10:8080/tr_reactnative/insert.php',{
      method:'POST',
      headers:{
      ...
      <View style={styles.container,{flex:1}}>
        ...
        <Text style={styles.txt}>Email:</Text>
          ...
          <TouchableOpacity
              style = {styles.submitButton}
              onPress={
                () => {this.props.navigation.navigate("Danh sách học sinh");
              }}
              >
              <Text style={ styles.submitButtonText}>Xem danh sách học sinh</Text>
          </TouchableOpacity>
        </View>
...
class DetailsScreen extends React.Component {
  ...
  Action_Click(id, name, email, phone_number){
    this.props.navigation.navigate("Cập nhật học sinh",{
      id: id,
      name: name,
      email: email,
  ...
  componentDidMount(){
    fetch('http://192.168.1.10:8080/tr_reactnative/getdataUsers.php')
    .then((response)=>response.json())
    .then((responseJson) => {
      this.setState({
        isLoading:false,
...
class UpdateDataUser extends React.Component {
  ...
  UpdateUser= () =>{
    fetch('http://192.168.1.10:8080/tr_reactnative/update.php', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
  ...
  DeleteUser= () =>{
    fetch('http://192.168.1.10:8080/tr_reactnative/delete.php', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    ...
    return (
      <NavigationContainer>
        <Stack.Navigator screenOptions={{
          ...
          <Stack.Screen name='Quản lý học sinh' component={HomeScreen} />
          <Stack.Screen name='Danh sách học sinh' component={DetailsScreen} />
          <Stack.Screen name='Cập nhật học sinh' component={UpdateDataUser} />
        </Stack.Navigator>
      </NavigationContainer>
    );
  }
